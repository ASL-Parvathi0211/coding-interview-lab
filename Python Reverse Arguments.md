...
For example, if the original function, f, is a pow function, f(2,3) = 8, 23 = 8, then the correct result is a function g, with g(3,2) = 9, because 32 = 9. Complete the function described below. Your function will be tested on 4 different functions included in the locked template code.

 
Function Description 
 
Complete the function reversed_args in the editor with the following parameter(s):

    function f: the function whose result needs to be computed with the order of arguments reversed

Returns

    function g: when called, returns the result of f called with the arguments reversed

Constraints

1 ≤ q ≤ 100
No function will be called with more than 100 arguments.
...
-----                                           --------
4                                             → integer q = 4
pow 2 3                                       → query[0]: function name, arguments  
cmp 1 2                                       → query[1]: function name, arguments
join_with coder best the are you ,            → query[2]: function name, arguments            
capitalize_first_and_join first second third  → query[3]: function name, arguments
Sample Output 0
...
Only pay attention to what the given functions should return, and call them with arguments provided in reverse order. The functions are:
